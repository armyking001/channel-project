...
def _scoped_query(db: Session, current_user: User):
    ...
    if current_user.role == UserRole.normal:
        ...
        child_ids = [c.id for c in current_user.children.all()]
...
def export(
    ...
    try:
        ...
        ws = wb.active
        ws.title = "项目明细"
        headers = ["项目编号", "项目名称", "合作单位", "合作模式", "费用模式",
                   "项目金额", "费用金额", "是否SM", "中标状态", "审批状态",
                   "投标日期", "开标日期", "创建人", "审批人", "创建时间"]
        ws.append(headers)
        coop_label = {"long_term": "长期", "short_term": "短期"}
        fee_label = {"mutual": "互免", "charged": "收费"}
        sm_label = {"yes": "是", "no": "否"}
        win_label = {"yes": "已中标", "no": "未中标", "in_progress": "进行中"}
        status_label = {"pending_submit": "待提交", "pending_approval": "待审批",
                        "approved": "已通过", "rejected": "已驳回"}
        for p in rows:
            ws.append([
                p.project_code, p.project_name, p.partner_company,
                ...
                float(p.project_amount or 0), float(p.fee_amount or 0),
        ...
        # 列宽
        for col, w in enumerate([14, 30, 24, 10, 10, 14, 14, 8, 10, 10, 12, 12, 10, 10, 16], 1):
