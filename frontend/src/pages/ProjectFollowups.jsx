...
export default function ProjectFollowups() {
  ...
  // 表单中用户动态添加的字段（自定义字段列表）
  const [extraFields, setExtraFields] = useState([])  // [{ id, label, key, type, value }]
  ...
  const removeExtraField = (id) => {
    setExtraFields(arr => arr.filter(f => f.id !== id))
    setForm(f => {
      const fd = { ...(f.form_data || {}) }
      delete fd[id]
      return { ...f, form_data: fd }
    })
  }
            ...
            <div className="p-6">
              ...
              {extraFields.length > 0 && (
                <div className="mt-4 border-t pt-4">
                  <div className="text-xs text-gray-500 mb-2">本次会话添加的临时字段：</div>
                      ...
                      <div key={ef.id} className="flex items-center gap-2">
                        <input
                          ...
                          onChange={e => {
                            ...
                            setExtraFields(arr => arr.map(x => x.id === ef.id ? { ...x, label: v } : x))
                          }}
                          className="w-32 px-2 py-1.5 border rounded text-sm"
                        />
                        <input
                          placeholder="字段值"
                          value={(form.form_data || {})[ef.id] || ''}
                          onChange={e => setForm(ff => ({
                            ...ff,
                            form_data: { ...(ff.form_data || {}), [ef.id]: e.target.value },
                          }))}
              ...
              <button
                ...
                className="mt-3 px-3 py-1.5 border border-dashed border-blue-400 text-blue-600 rounded text-sm hover:bg-blue-50"
              >+ 添加字段</button>
